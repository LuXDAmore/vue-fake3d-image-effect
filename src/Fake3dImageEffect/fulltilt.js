...
sensors.orientation.callbacks[ i ].call(
 this 
);
...
function handleDeviceMotionChange(
 event 
) {
...
sensors.motion.callbacks[ i ].call(
 this 
);
...
FULLTILT.getDeviceOrientation = function(
 options 
) {
 ...
 resolve, reject 
		...
		var control = new FULLTILT.DeviceOrientation(
 options 
);

		control.start();

		var orientationSensorCheck = new SensorCheck(
 sensors.orientation 
);

		orientationSensorCheck.then(
 function() {
...
FULLTILT.getDeviceMotion = function(
 ...
 resolve, reject 
		...
		var control = new FULLTILT.DeviceMotion(
 options 
);

		control.start();

		var motionSensorCheck = new SensorCheck(
 sensors.motion 
);

		motionSensorCheck.then(
 function() {
...
1, 
 ...
 outQuat 
	...
	this.normalize = function() {
		...
		return FULLTILT.Quaternion.prototype.normalize(
 this 
);
...
sA;
 ...
 targetQuaternion, transformQuaternion 
);
...
sqz;
 ...
 m 
		...
		outMatrix = FULLTILT.RotationMatrix.prototype.multiplyMatrices(
 this, m 
);
		this.copy(
 outMatrix 
...
1, 
 ...
 outMatrix 
	...
	this.normalize = function() {
		...
		return FULLTILT.RotationMatrix.prototype.normalize(
 this 
);
...
bE;
 ...
 matrix 
		...
		// Calculate matrix determinant
		 determinant = R[ 0 ] * R[ 4 ] * R[ 8 ] - R[ 0 ] * R[ 5 ] * R[ 7 ] - R[ 1 ] * R[ 3 ] * R[ 8 ] + R[ 1 ] * R[ 5 ] * R[ 6 ] + R[ 2 ] * R[ 3 ] * R[ 7 ] - R[ 2 ] * R[ 4 ] * R[ 6 ];
...
cA,
		...
		return function(
 targetRotationMatrix, axis, angle 
) {
 ...
 targetRotationMatrix, transformMatrix 
);
...
_gamma;
 ...
 q 
			...
			var sqw = q.w * q.w,
			 ...
			 unitLength = sqw + sqx + sqy + sqz, // Normalised == 1, otherwise correction divisor.
 ...
 angle 
		...
		FULLTILT.Euler.prototype.rotateByAxisAngle(
 this, [
 1,
0,
0, 
...
FULLTILT.Euler.prototype = {
	...
	rotateByAxisAngle: function() {
		...
		return function(
 targetEuler, axis, angle 
) {

			_matrix.setFromEuler(
 targetEuler 
);
...
FULLTILT.DeviceOrientation = function(
 options 
) {
	...
	if( this.options.type === 'game' ) {
		...
		var setGameAlphaOffset = function(
 evt 
) {
 ...
 evt.alpha, 0, 0 
				...
				this.alphaOffsetDevice.rotateZ(
 ...
 'deviceorientation', setGameAlphaOffset, false 
);
					return;
...
window.removeEventListener(
 ...
 'deviceorientation', setGameAlphaOffset, false 
	...
	// Create a compass-based deviceorientation object (initial alpha === compass degrees)
		...
		var setCompassAlphaOffset = function(
 evt 
) {
 ...
 screenOrientationAngle 
				...
				if( ++ successCount >= successThreshold ) {
					...
					window.removeEventListener(
 'deviceorientation', setCompassAlphaOffset, false 
);
					return;
...
FULLTILT.DeviceOrientation.prototype = {
 ...
 callback 
		...
		if( callback && Object.prototype.toString.call(
 callback 
) == '[object Function]' ) {

sensors.orientation.callbacks.push(
 callback 
);
...
window.screen.orientation.addEventListener(
 'change', handleScreenOrientationChange, false 
);

} else {

window.addEventListener(
 'orientationchange', handleScreenOrientationChange, false 
);
		...
		if( ! sensors.orientation.active ) {
			...
			window.addEventListener(
 'deviceorientation', handleDeviceOrientationChange, false 
);
...
window.addEventListener(
 'orientationchange', handleScreenOrientationChange, false 
		...
		if( ! sensors.motion.active ) {
			...
			window.addEventListener(
 'devicemotion', handleDeviceMotionChange, false 
);
